...
|---|---|---|---|
| [genieutils-py](https://github.com/SiegeEngineers/genieutils-py) | LGPLv3 | Yes | See the obligations below. Its metadata states no licence, so `tools/third_party_notices.py` carries it explicitly. |
...
| Pillow | MIT-CMU | Yes | Confirmed from the installed metadata, 12.3.0, 2026-09-16. Wheels bundle native libraries with their own notices. Used only by `capture.icons` and `store.icons`. |
...
- [ ] `THIRD_PARTY_NOTICES`: every bundled package with version, licence, copyright notice and source URL, including Pillow's bundled native libraries and CPython. Generated by `uv run python tools/third_party_notices.py`, then reviewed by hand; the release workflow fails if the committed file is out of date.
